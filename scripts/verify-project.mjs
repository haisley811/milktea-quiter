...
assert.match(profileView, /账户与个性化/);
assert.match(profileView, /本地模式/);
assert.match(profileView, /无需登录/);
...
assert.match(recordView, /记录信息/);
assert.match(recordView, /智能分析/);
...
assert.match(pillButton, /min-h-11/);

const formRow = file("components/FormRow.tsx");
assert.match(formRow, /hover:bg-white\/45/);
assert.match(formRow, /h-2 w-2 shrink-0 rounded-full/);
...
assert.match(preview, /\.stack \{ gap: 16px/);
...
assert.match(preview, /max-width: 480px/);
...
assert.match(preview, /账户与个性化/);
assert.match(preview, /无需登录/);
